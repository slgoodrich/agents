...
```
/pm:impact [feature or initiative] [target metrics]
```
...
- `/pm:mvp` - Scope MVP based on impact estimates
- `/pm:prioritize` - Prioritize features by impact
- `/pm:experiment` - Test impact with experiments before full build
- `/pm:kpi-dashboard` - Track metrics mentioned in impact estimate
- `/pm:okr` - Connect feature impact to OKR progress
- `/workflows:discovery-sprint` - Validate impact assumptions before building
